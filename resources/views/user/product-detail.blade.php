...
.similar-card:hover { border-color: rgba(212,175,55,0.2); transform: translateY(-4px); box-shadow: 0 20px 40px rgba(0,0,0,0.5); }
...
<section class="min-h-screen py-16 md:py-24 px-4">
    <div class="max-w-7xl mx-auto">
        <div class="flex items-center gap-3 text-[0.5rem] uppercase tracking-[0.3em] text-[#666] mb-10" data-aos="fade-down">
            <a href="{{ route('user.home') }}" class="hover:text-[#d4af37] transition-all">Home</a>
            <span>/</span>
            <a href="{{ route('user.shop') }}" class="hover:text-[#d4af37] transition-all">Shop</a>
            <span>/</span>
            <span class="text-[#a1a1aa]">{{ $product->name }}</span>
        </div>

        <div class="grid grid-cols-1 lg:grid-cols-2 gap-12 lg:gap-16">
            <div data-aos="fade-right">
                <div class="relative bg-[#0a0a0e] rounded-2xl overflow-hidden border border-[rgba(255,255,255,0.04)] group">
                    <div class="aspect-square relative overflow-hidden">
                        {{-- Already uses Tailwind crossfade --}}
                    ...
                    <div class="absolute top-6 left-6">
                        <span class="text-[0.5rem] uppercase tracking-[0.2em] font-semibold px-4 py-2 rounded-full bg-red-500/20 text-red-400 border border-red-500/30">Out of Stock</span>
                    </div>
                    @elseif($product->stock <= 3)
                    <div class="absolute top-6 left-6">
                        <span class="text-[0.5rem] uppercase tracking-[0.2em] font-semibold px-4 py-2 rounded-full bg-amber-500/20 text-amber-400 border border-amber-500/30">Only {{ $product->stock }} Left</span>
                    </div>
                    @endif
                </div>
            </div>
            ...
            <div class="flex flex-col justify-center" data-aos="fade-left">
                <div class="space-y-6">
                    <div class="flex items-center gap-3">
                        <span class="text-[0.5rem] uppercase tracking-[0.3em] text-[#a1a1aa] font-medium">{{ $product->brand ?? 'Valencia' }}</span>
                        @if($product->category)
                        <span class="w-px h-3 bg-[rgba(255,255,255,0.04)]"></span>
                        <span class="text-[0.5rem] uppercase tracking-[0.3em] text-[#d4af37]">{{ $product->category->name }}</span>
                        @endif
                    </div>
                    ...
                    <div class="flex items-baseline gap-4">
                        <span class="text-3xl font-light text-[#d4af37]">${{ number_format($product->price, 2) }}</span>
                        @if($product->stock > 0)
                    ...
                    <form action="{{ route('user.cart.add', $product) }}" method="POST" class="flex items-center gap-4 pt-4">
                        @csrf
                        <div class="flex items-center gap-3 bg-[#0a0a0e] border border-[rgba(255,255,255,0.04)] rounded-full px-4 py-2">
                    ...
                    <div class="flex items-center gap-6 pt-4 text-[0.4rem] uppercase tracking-[0.3em] text-[#666]">
...
<section class="pb-24 px-4">
    <div class="max-w-7xl mx-auto">
        <div class="mb-12 text-center" data-aos="fade-up">
            <span class="text-[9px] uppercase tracking-[0.5em] text-[#d4af37] font-medium block mb-4">Curated Selection</span>
            <h2 class="luxury-title text-3xl md:text-4xl font-light text-white tracking-wide">
                Complete Your <span class="text-[#d4af37]">Collection</span>
            </h2>
            <div class="w-12 h-px bg-gradient-to-r from-transparent via-[#d4af37] to-transparent mx-auto mt-4"></div>
        </div>

        <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-5">
            @foreach($similar as $item)
            <a href="{{ route('user.product.detail', $item) }}" class="similar-card group" data-aos="fade-up">
                <div class="aspect-square overflow-hidden bg-[#050507]">
                    {{-- Crossfade using Tailwind for similar products too --}}
                ...
                <div class="p-5 space-y-2">
                    <div class="flex items-center justify-between">
                        <span class="text-[0.5rem] uppercase tracking-[0.2em] text-[#a1a1aa] font-medium">{{ $item->brand ?? 'Valencia' }}</span>
                        <span class="text-xs font-medium text-[#d4af37]">${{ number_format($item->price, 2) }}</span>
                    </div>
                    <h3 class="text-sm font-light text-white tracking-wide">{{ $item->name }}</h3>
                </div>
            </a>
            @endforeach
        </div>
